...
- 右下角紫色圆点「中」→ 打开菜单：三个功能开关 + 翻译引擎选择（自动 / 仅本地 / 仅 Google），底部显示当前翻译状态与缓存量。
- 「界面汉化」切换会刷新页面；另外两项即时生效。
- 设置存在 j7tracker.io 的 localStorage：`j7zh_enabled` / `j7zh_reader` / `j7zh_autotrans` / `j7zh_engine`，译文缓存在 `j7zh_trcache_v1`（约 1500 条滚动）。
